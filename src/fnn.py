...
class TestInstance(unittest.TestCase):
	...
	def test_loss_grad(self):
		...
		grad = grad_softmax_crossentropy_with_logits(y_true.astype(np.int32, y)
...
if __name__ == "__main__":
    ...
    model = _SimpleNN.FNN(config2)
    model.summary()

    x_train = np.random.random(size=(100, 10))
    y_train = np.random.randint(0,4,100).astype(np.float64).reshape(-1, 1)
